...
int InitCameras() 
{
  try {
				...
				std::cout << "Camera type in group A is "<< mvcam[0]->GetCameraType(group_a_key) << std::endl;
				}
				if (GROUP_NUM == 2)  // B组
				{
				...
				std::cout << "Camera type in group B is "<< mvcam[1]->GetCameraType(group_b_key) << std::endl;
				}
  		}
...
int GetPixels(unsigned char** buf) 
{
  try
  {
		...
		// if(GROUP_NUM<=1)    //A组
		{
			...
			if (mvcam[0]->GetImagePtr(group_a_buf, timestamp)) 
			{
			// outbuf : front back left right
			for (int i = 0; i < GROUP_A_CAMERA_NUM; i++) buf[i] = group_a_buf[i];
			// buf[1] = group_a_buf[0];
			// buf[2] = group_a_buf[0];
			// buf[3] = group_a_buf[0];
			// buf[4] = group_a_buf[0];
		...
		if (GROUP_NUM == 2)  // B组
		{
			...
			// camera : front back left right
			if (mvcam[1]->GetImagePtr(group_b_buf, timestamp)) 
			{
			// outbuf : front back left right
