...
vec3 *verCoordPoint;
vec2 *texCoordPoint, *imgCoordPoint;
...
GLuint bindTexture(GLuint texture, unsigned char *buffer, GLuint w, GLuint h,
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, type /*GL_LUMINANCE*/, w, h, 0,
               type /*GL_LUMINANCE*/, GL_UNSIGNED_BYTE, buffer);
...
char fragmentSource[] = {
    ...
    "void main()                                                 \n"
    "{                                                           \n"
    "vec3 origin = texture(textureRGBA, vv2texcoord).rgb;\n"
...
char fragmentMosaicSource[] = {
    "#version 300 es\n"
    "precision highp float;                                         \n"
    "uniform sampler2D texture0RGBA;                                 \n"
    "uniform sampler2D texture1RGBA;                                 \n"
    "uniform vec3 color0Adjust;                                   \n"
    "uniform vec3 color1Adjust;                                   \n"
    ...
    "void main()                                                    \n"
    "{                                                              \n"
    "    vec3 rgb0 = texture(texture0RGBA, vv2texcoord0).rgb;        \n"
    "    vec3 rgb1 = texture(texture1RGBA, vv2texcoord1).rgb;        \n"
    "    fragColor = vec4(mix(rgb0 + color0Adjust, rgb1 + color1Adjust, "
    "vvalpha), 1.0);\n"
    "}                                                              \n"};
#else
char fragmentMosaicSource[] = {
    ...
    "1, -0.3455, -0.7169,\n"
    "1, 0, 1.4075);\n"
    "    fragColor = vec4(mix(rgbColor0 + color0Adjust, rgbColor1 + "
    "color1Adjust, vvalpha), 1.0);\n"
    "}                                                              \n"};
#endif
...
char fragmentCameraSource[] = {
    ...
    "void main()                                                 \n"
    "{                                                           \n"
    "    fragColor = texture(textureRGBA, vv2texcoord);        \n"
    "}                                                           \n"};
#else
char fragmentCameraSource[] = {
    "#version 300 es \n"
...
void caculateColorCoeff2D(unsigned char **imageBuffer, int flag) {
  ...
  for (i = 0; i < texCoordsStatistics2D[flag].glTexCoord_FL_F.size(); i++) {
    index0 = texCoordsStatistics2D[flag].glTexCoord_FL_F[i] * 4;
    rSum0 += imageBuffer[0][index0 + 0];
    gSum0 += imageBuffer[0][index0 + 1];
    bSum0 += imageBuffer[0][index0 + 2];

    index1 = texCoordsStatistics2D[flag].glTexCoord_FL_L[i] * 4;
  ...
  for (i = 0; i < texCoordsStatistics2D[flag].glTexCoord_FR_F.size(); i++) {
    index0 = texCoordsStatistics2D[flag].glTexCoord_FR_F[i] * 4;
    rSum0 += imageBuffer[0][index0 + 0];
    gSum0 += imageBuffer[0][index0 + 1];
    bSum0 += imageBuffer[0][index0 + 2];

    index1 = texCoordsStatistics2D[flag].glTexCoord_FR_R[i] * 4;
  ...
  for (i = 0; i < texCoordsStatistics2D[flag].glTexCoord_BL_B.size(); i++) {
    index0 = texCoordsStatistics2D[flag].glTexCoord_BL_B[i] * 4;
    rSum0 += imageBuffer[1][index0 + 0];
    gSum0 += imageBuffer[1][index0 + 1];
    bSum0 += imageBuffer[1][index0 + 2];

    index1 = texCoordsStatistics2D[flag].glTexCoord_BL_L[i] * 4;
    rSum1 += imageBuffer[2][index1 + 0];
    gSum1 += imageBuffer[2][index1 + 1];
    bSum1 += imageBuffer[2][index1 + 2];
  }
  ...
  for (i = 0; i < texCoordsStatistics2D[flag].glTexCoord_BR_B.size(); i++) {
    index0 = texCoordsStatistics2D[flag].glTexCoord_BR_B[i] * 4;
    rSum0 += imageBuffer[1][index0 + 0];
    gSum0 += imageBuffer[1][index0 + 1];
    bSum0 += imageBuffer[1][index0 + 2];

    index1 = texCoordsStatistics2D[flag].glTexCoord_BR_R[i] * 4;
    rSum1 += imageBuffer[3][index1 + 0];
    gSum1 += imageBuffer[3][index1 + 1];
    bSum1 += imageBuffer[3][index1 + 2];
  }
  ...
  // for(i=0; i<12; i++)
  // printf("count = %d, idx=%d ===== %f %f %f\n", runCount, i, colorAve[i].x,
  // colorAve[i].y, colorAve[i].z);
}
...
GLuint esLoadShader(GLenum type, const char *shaderSrc) {
  ...
  if (!compiled) {
    ...
    if (infoLen > 1) {
      infoLog = reinterpret_cast<char *>(malloc(sizeof(char) * infoLen));
...
?$)A(e????:
$)Ah>?????:
vertShaderSrc:$)Ai!6g????2e?f:??o<?ragShaderSrc:??????2e?f:??
$)Ah???g;??:   ??;:e%=g??%f?
$)Ad???h.0e?o<?
====================================================================*/
...
static void setVec3(GLuint programId, const char *name, float x, float y,
                    float z) {
...
static void setVec4(GLuint programId, const char *name, float x, float y,
                    float z, float w) {
...
$)Ad???h.0e?o<?
====================================================================*/
void rotationMatrixToEulerAngle(float *R, float *alpha, float *beta,
                                float *gamma) {
...
static int initShader(void) {
  ...
  hProgramHandle[2] =
      esLoadProgram(vertexBmpShowSource, fragmentBmpShowSource);  // show car
  ...
  printf("%d %d %d %d %d\n", hProgramHandle[0], hProgramHandle[1],
         hProgramHandle[2], hProgramHandle[3], hProgramHandle[4]);
...
void show2DCar(int flag) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].CamVerticesPoints[0]);
  ...
  setInt(hProgramHandle[0], "textureRGBA", 0);

  glDrawArrays(GL_TRIANGLE_STRIP, 0,
               texCoords2D[flag].glTexCoord_F.size());  // GL_TRIANGLE_STRIP

  // back
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].CamVerticesPoints[1]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  // left
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].CamVerticesPoints[2]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  // right
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].CamVerticesPoints[3]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicCamVerticesPoints[0]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicFLCamImagePoints[0]);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicFLCamImagePoints[1]);
  ...
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, textureYUYV[0]);
  setInt(hProgramHandle[1], "texture0RGBA", 0);

  glActiveTexture(GL_TEXTURE1);
  glBindTexture(GL_TEXTURE_2D, textureYUYV[2]);
  setInt(hProgramHandle[1], "texture1RGBA", 1);
  ...
  glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicCamVerticesPoints[1]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicFRCamImagePoints[0]);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicFRCamImagePoints[1]);
  glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicCamVerticesPoints[2]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicBLCamImagePoints[0]);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicBLCamImagePoints[1]);
  glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicCamVerticesPoints[3]);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicBRCamImagePoints[0]);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
  glBindBuffer(GL_ARRAY_BUFFER,
               VBO2DMosaicImageParams[flag].MosaicBRCamImagePoints[1]);
  glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, 0);
...
void drawUndistortBackCurve(ESMatrix orthoMatrix) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    glBufferData(GL_ARRAY_BUFFER, LENGTH * 2 * sizeof(vec3),
                 verticesRearTrajLinePoint[i], GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  glBindBuffer(GL_ARRAY_BUFFER, curveVerticesPoints[0]);
  glBufferData(GL_ARRAY_BUFFER, LENGTH * 2 * sizeof(vec3),
               verticesRearTrajLinePoint[6], GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  glBindBuffer(GL_ARRAY_BUFFER, curveVerticesPoints[0]);
  glBufferData(GL_ARRAY_BUFFER, LENGTH * 2 * sizeof(vec3),
               verticesRearTrajLinePoint[7], GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);

  // glDrawArrays(GL_TRIANGLE_STRIP, 0, LENGTH * 2);
}

vec2 undistort(float i, float j, const float *intrinsic_matrix,
               const float *dist_coeffs, undistortParams params, double *invR) {
  float u, v;
  double x, y, x2, y2, r2, r, r4, r6, theta, theta2, theta4, theta6, theta8,
      theta_d, xd, yd, kr, _2xy;
...
void showSingleView(int viewMode, float wheelAngle, int flag) {
  ...
  if (viewMode == VIEW_BACKWARD) {
    camera[0] = static_cast<float>(backWardCamParams[flag].mimdInt[0]) / SCALE2;
    camera[1] = static_cast<float>(backWardCamParams[flag].mimdInt[1]) / SCALE2;
    camera[2] = static_cast<float>(backWardCamParams[flag].mimdInt[2]) / SCALE2;
    camera[3] = static_cast<float>(backWardCamParams[flag].mimdInt[3]) / SCALE2;

    distortTable[0] =
        static_cast<float>(backWardCamParams[flag].mimdInt[4]) / SCALE1;
    distortTable[1] =
        static_cast<float>(backWardCamParams[flag].mimdInt[5]) / SCALE1;
    distortTable[2] =
        static_cast<float>(backWardCamParams[flag].mimdInt[6]) / SCALE1;
    distortTable[3] =
        static_cast<float>(backWardCamParams[flag].mimdInt[7]) / SCALE1;
    distortTable[4] = 0.003176562358955857;

    rVec[0] = static_cast<float>(backWardCamParams[flag].mrInt[0]) / SCALE3;
    rVec[1] = static_cast<float>(backWardCamParams[flag].mrInt[1]) / SCALE3;
    rVec[2] = static_cast<float>(backWardCamParams[flag].mrInt[2]) / SCALE3;

    tVec[0] = static_cast<float>(backWardCamParams[flag].mtInt[0]) / SCALE2;
    tVec[1] = static_cast<float>(backWardCamParams[flag].mtInt[1]) / SCALE2;
    tVec[2] = static_cast<float>(backWardCamParams[flag].mtInt[2]) / SCALE2;
    ...
    resizer.x = 110;
    resizer.y = 130;
    resizer.xZoom = 1.0 * 1060 / bvs3DWidth;
    resizer.yZoom = 1.0 * 620 / bvs3DHeight;
    ...
    /*rMat[0] = -0.996580; rMat[1] = -0.072751; rMat[2] = 0.039182;
    rMat[3] = 0.050573; rMat[4] = -0.162013; rMat[5] = 0.985492;
    rMat[6] = -0.065347; rMat[7] = 0.984103; rMat[8] = 0.165138;

    cvRodrigues2(&rMAT,  &rVEC, NULL );

    printf("%d %d %d\n", cvRound(rVec[0] * SCALE3), cvRound(rVec[1] * SCALE3),
    cvRound(rVec[2] * SCALE3));*/
  }
  ...
  glBufferData(GL_ARRAY_BUFFER, cnt * sizeof(vec3), verAddr, GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);

  glBindBuffer(GL_ARRAY_BUFFER, cameraVerTexCoord[1]);
  glBufferData(GL_ARRAY_BUFFER, cnt * sizeof(vec2), texAddr, GL_STATIC_DRAW);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  if (viewMode == VIEW_BACKWARD) {
    findRearCurve(wheelAngle, resizer, camera, distortTable, rMat, tVec, invR,
                  bvs3DWidth, bvs3DHeight);
...
void showFourView() {
  ...
  // Front
  glBindBuffer(GL_ARRAY_BUFFER, cameraVerTexCoord[0]);
  glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(vec3), verticesViewFront,
               GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);

  glBindBuffer(GL_ARRAY_BUFFER, cameraVerTexCoord[1]);
  glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(vec2), texCoordView, GL_STATIC_DRAW);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  // Back
  glBindBuffer(GL_ARRAY_BUFFER, cameraVerTexCoord[0]);
  glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(vec3), verticesViewBack,
               GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  // Left
  glBindBuffer(GL_ARRAY_BUFFER, cameraVerTexCoord[0]);
  glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(vec3), verticesViewLeft,
               GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
  ...
  // Right
  glBindBuffer(GL_ARRAY_BUFFER, cameraVerTexCoord[0]);
  glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(vec3), verticesViewRight,
               GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
...
void initBackWard(int width, int height) {
  ...
  imgCoordPoint = (vec2 *)malloc(sizeof(vec2) * p * (p + 1) * 2);

  verCount = generateTriangle(imgCoordPoint, width, height);

  texCoordPoint = (vec2 *)malloc(sizeof(vec2) * verCount);
  verCoordPoint = (vec3 *)malloc(sizeof(vec3) * verCount);
